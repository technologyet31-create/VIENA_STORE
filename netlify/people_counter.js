(function () {
  ...
  // Optional controls (used in Netlify build). If missing, defaults keep current behavior.
  const apiBaseInput = document.getElementById('apiBase');
  const enablePeopleChk = document.getElementById('enablePeople');
  ...
  const lastFrame = { w: 0, h: 0 };
  let peopleBackendDisabled = false;

  function getApiBase() {
    const params = new URLSearchParams(location.search);
    const fromQuery = (params.get('api') || '').trim();
    const fromInput = (apiBaseInput && apiBaseInput.value || '').trim();
    const base = fromQuery || fromInput;
    return base.replace(/\/+$/, '');
  }

  function isPeopleEnabled() {
    if (peopleBackendDisabled) return false;
    if (!enablePeopleChk) return true;
    return !!enablePeopleChk.checked;
  }

  function syncFaceHandBoxes() {
    const targetW = lastFrame.w;
    const targetH = lastFrame.h;
  ...
  function runMediaPipe(imageEl, timestampMs) {
    ...
    Promise.resolve()
      .then(() => {
        ...
        // Update overlay even if the Python backend is disabled/unavailable.
        syncFaceHandBoxes();
        if (lastFrame.w && lastFrame.h) {
          redrawAll(lastFrame.w, lastFrame.h);
        }
      })
      .catch((e) => {
        mpLastError = String(e && (e.message || e) || e);
  ...
  async function captureAndSend() {
    ...
    try {
      ...
      // If we're not using the backend, just draw face/hand overlays.
      if (!isPeopleEnabled()) {
        if (countPeopleEl) countPeopleEl.textContent = '—';
        syncFaceHandBoxes();
        redrawAll(targetW, targetH);
        const mpNote = mpLastError ? ` | MediaPipe: ${mpLastError}` : '';
        setStatus(`يعمل الآن (بدون بايثون). وجوه: ${countFacesEl?.textContent || '—'} | أيدي: ${countHandsEl?.textContent || '—'}${mpNote}`);
        return;
      }
      ...
      const url = apiBase ? `${apiBase}/count` : '/count';
      ...
      // Convert face/hand boxes into pixel boxes.
      syncFaceHandBoxes();
      ...
      console.error(err);
      // If we're on a static host (Netlify) without a backend, avoid spamming a hard error.
      if (apiBaseInput || enablePeopleChk) {
        peopleBackendDisabled = true;
        if (countPeopleEl) countPeopleEl.textContent = '—';
        setStatus('تعذر الاتصال بسيرفر بايثون. سيتم متابعة الوجوه/الأيدي فقط. (ضع رابط API صحيح إذا أردت عدّ الأشخاص)');
      } else {
  ...
  if (enablePeopleChk) {
    enablePeopleChk.addEventListener('change', () => {
      peopleBackendDisabled = false;
      if (!enablePeopleChk.checked && countPeopleEl) countPeopleEl.textContent = '—';
    });
  }
